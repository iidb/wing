...
std::function<StaticFieldRef(U, T...)> GenerateStringColumnExpr(uint32_t offset) {
...
template <typename V>
StaticFieldRef ReadFromAggregateIntermediateData(V t, uint32_t id, StaticFieldRef) {
  return t[id].data_.int_data;
}
...
std::function<RetValueType(U, V, T...)> GenerateColumnExpr2(uint32_t id) {
  return [=](U, V t, T...) { return ReadFromAggregateIntermediateData(t, id, RetValueType()); };
}

template <typename RetValueType, typename U, typename V, typename... T>
std::function<RetValueType(U, V, T...)> GenerateColumnAvgExpr2(uint32_t id) {
  return [=](U, V t, T...) { return RetValueType(t[id].data_.double_data / t[id].size_); };
}
...
std::function<RetValueType(T...)> GenerateExprFunction(
    const Expr* expr, const OutputSchema& input_schema, const OutputSchema* right_input_schema = nullptr,
    std::vector<const Expr*>* aggregate_expr_vec = nullptr,
    std::vector<std::function<void(AggregateIntermediateData&, StaticFieldRef)>>* aggregate_func_vec = nullptr);

template <typename... T>
std::function<StaticFieldRef(T...)> GenerateStringExprFunction(
    const Expr* expr, const OutputSchema& input_schema, const OutputSchema* right_input_schema = nullptr,
    std::vector<const Expr*>* aggregate_expr_vec = nullptr,
    std::vector<std::function<void(AggregateIntermediateData&, StaticFieldRef)>>* aggregate_func_vec = nullptr);
...
template <typename RetValueType, typename... T>
std::function<RetValueType(T...)> GenerateExprFunction(
    const Expr* expr, const OutputSchema& input_schema, const OutputSchema* right_input_schema, std::vector<const Expr*>* aggregate_expr_vec,
    std::vector<std::function<void(AggregateIntermediateData&, StaticFieldRef)>>* aggregate_func_vec) {
  if (expr->type_ == ExprType::LITERAL_INTEGER) {
    return [x = static_cast<const LiteralIntegerExpr*>(expr)->literal_value_](T...) { return static_cast<RetValueType>(x); };
  } else if (expr->type_ == ExprType::LITERAL_FLOAT) {
    return [x = static_cast<const LiteralFloatExpr*>(expr)->literal_value_](T...) { return static_cast<RetValueType>(x); };
  } else if (expr->type_ == ExprType::UNARYOP) {
    auto this_expr = static_cast<const UnaryExpr*>(expr);
    auto ch =
        GenerateExprFunction<RetValueType, T...>(this_expr->ch0_.get(), input_schema, right_input_schema, aggregate_expr_vec, aggregate_func_vec);
...
#define GEN_FUNC(op_name, op)                \
  {                                          \
    ...
    if (this_expr->ch0_->ret_type_ == RetType::FLOAT) {
      auto ch0 = GenerateExprFunction<double, T...>(this_expr->ch0_.get(), input_schema, right_input_schema, aggregate_expr_vec, aggregate_func_vec);
      auto ch1 = GenerateExprFunction<double, T...>(this_expr->ch1_.get(), input_schema, right_input_schema, aggregate_expr_vec, aggregate_func_vec);
      GEN_FUNC(ADD, +);
      GEN_FUNC(SUB, -);
      GEN_FUNC(MUL, *);
      GEN_FUNC(DIV, /);
      ...
      auto ch0 = GenerateExprFunction<int64_t, T...>(this_expr->ch0_.get(), input_schema, right_input_schema, aggregate_expr_vec, aggregate_func_vec);
      auto ch1 = GenerateExprFunction<int64_t, T...>(this_expr->ch1_.get(), input_schema, right_input_schema, aggregate_expr_vec, aggregate_func_vec);
      GEN_FUNC(ADD, +);
      GEN_FUNC(SUB, -);
      GEN_FUNC(MUL, *);
      GEN_FUNC(DIV, /);
    ...
    if (this_expr->ch0_->ret_type_ == RetType::STRING) {
      auto ch0 = GenerateStringExprFunction<T...>(this_expr->ch0_.get(), input_schema, right_input_schema, aggregate_expr_vec, aggregate_func_vec);
      auto ch1 = GenerateStringExprFunction<T...>(this_expr->ch1_.get(), input_schema, right_input_schema, aggregate_expr_vec, aggregate_func_vec);
#define GEN_STR_FUNC(op_name, op)                                                                                         \
  {                                                                                                                       \
    if (this_expr->op_ == OpType::op_name) {                                                                              \
      return [=](T... t) { return static_cast<RetValueType>(ch0(t...).ReadStringView() op ch1(t...).ReadStringView()); }; \
    }                                                                                                                     \
  }
      GEN_STR_FUNC(LT, <);
      GEN_STR_FUNC(GT, >);
...
#define AGGR_FUNC(statement) aggregate_func_vec->push_back([](AggregateIntermediateData& x, StaticFieldRef y) { statement; })
    if (this_expr->func_name_ == "max") {
      if (this_expr->ret_type_ == RetType::INT) {
        AGGR_FUNC({ x.data_.int_data = y.data_.int_data; });
        AGGR_FUNC({ x.data_.int_data = std::max(x.data_.int_data, y.data_.int_data); });
      } else if (this_expr->ret_type_ == RetType::FLOAT) {
        AGGR_FUNC({ x.data_.double_data = y.data_.double_data; });
        AGGR_FUNC({ x.data_.double_data = std::max(x.data_.double_data, y.data_.double_data); });
      }
    } else if (this_expr->func_name_ == "min") {
      if (this_expr->ret_type_ == RetType::INT) {
        AGGR_FUNC({ x.data_.int_data = y.data_.int_data; });
        AGGR_FUNC({ x.data_.int_data = std::min(x.data_.int_data, y.data_.int_data); });
      } else if (this_expr->ret_type_ == RetType::FLOAT) {
        AGGR_FUNC({ x.data_.double_data = y.data_.double_data; });
        AGGR_FUNC({ x.data_.double_data = std::min(x.data_.double_data, y.data_.double_data); });
      }
      ...
      if (this_expr->ret_type_ == RetType::INT) {
        ...
        AGGR_FUNC({ x.data_.double_data = y.data_.double_data; });
        AGGR_FUNC({ x.data_.double_data = x.data_.double_data + y.data_.double_data; });
      }
    ...
    if (this_expr->func_name_ == "max" || this_expr->func_name_ == "min" || this_expr->func_name_ == "sum") {
    ...
    if (id != (~0u)) {
      if (!input_schema.IsRaw() || right_input_schema != nullptr) {
        ...
        return GenerateColumnExpr<RetValueType, T...>(id * sizeof(StaticFieldRef) * flag_join);
      } else {
        // RetValueType is not string
        // And we ensure that strings are behind static fields.
        // So we can use the offset directly.
        // Tuple layout: (tuple size 4 byte), (static fields), (offsets of varchars), (varchars)
        if (input_schema[id].type_ == FieldType::INT32) {
          return GenerateColumnExpr<int32_t, T...>(Tuple::GetOffsetOfStaticField(offset) * flag_join);
        }
        return GenerateColumnExpr<RetValueType, T...>(Tuple::GetOffsetOfStaticField(offset) * flag_join);
      }
    }
    ...
    if (right_input_schema != nullptr) {
      ...
      // Join use the last bit of offset to identify the table that the field is in.
      // i.e. offset = (the true offset) * 2 + (flag)
      offset = 0;
    ...
    if (!right_input_schema->IsRaw()) {
      return GenerateColumnExpr<RetValueType, T...>(id * sizeof(StaticFieldRef) * 2 + 1);
    } else {
      if ((*right_input_schema)[id].type_ == FieldType::INT32) {
        return GenerateColumnExpr<int32_t, T...>(Tuple::GetOffsetOfStaticField(offset) * 2 + 1);
      }
      return GenerateColumnExpr<RetValueType, T...>(Tuple::GetOffsetOfStaticField(offset) * 2 + 1);
    }
    ...
    if (expr->ret_type_ == RetType::FLOAT && this_expr->ch0_->ret_type_ == RetType::INT) {
      auto ch = GenerateExprFunction<int64_t, T...>(this_expr->ch0_.get(), input_schema, right_input_schema, aggregate_expr_vec, aggregate_func_vec);
      return [=](T... t) { return static_cast<RetValueType>(ch(t...)); };
    } else if (expr->ret_type_ == RetType::INT && this_expr->ch0_->ret_type_ == RetType::FLOAT) {
      auto ch = GenerateExprFunction<double, T...>(this_expr->ch0_.get(), input_schema, right_input_schema, aggregate_expr_vec, aggregate_func_vec);
      return [=](T... t) { return static_cast<RetValueType>(ch(t...)); };
    } else
...
template <typename... T>
std::function<StaticFieldRef(T...)> GenerateStringExprFunction(const Expr* expr, const OutputSchema& input_schema,
  ...
  if (expr->type_ == ExprType::LITERAL_STRING) {
    ...
    auto this_expr = static_cast<const ColumnExpr*>(expr);
    uint32_t id = ~0u, offset = 0, id_in_str = 0, flag_join = right_input_schema ? 2 : 1;
    for (uint32_t index = 0; auto& col : input_schema.GetCols()) {
      if (col.id_ == this_expr->id_in_column_name_table_) {
        id = index;
        break;
    ...
    if (id != (~0u)) {
      if (!input_schema.IsRaw() || right_input_schema != nullptr) {
        // This means that the input is an array of StaticFieldRef.
        return GenerateColumnExpr<StaticFieldRef, T...>(id * sizeof(StaticFieldRef) * flag_join);
      } else {
        // Tuple layout: (tuple size 4 byte), (static fields), (offsets of varchars), (varchars)
        // We must find the offsets of varchars.
        // Then find the corresponding offset.

        // This function requires the offset of the entry in the offsets of varchars.
        return GenerateStringColumnExpr<T...>(Tuple::GetOffsetsOfStrings(offset, id_in_str) * flag_join);
      }
    }
    ...
    if (!right_input_schema->IsRaw()) {
      return GenerateColumnExpr<StaticFieldRef, T...>(id * sizeof(StaticFieldRef) * 2 + 1);
    } else {
      return GenerateStringColumnExpr<T...>(Tuple::GetOffsetsOfStrings(offset, id_in_str) * 2 + 1);
    }
...
/**
 * @brief Generate a std::function object from an Expr. The std::function receives an iterator to parameters and returns the result as a Field.
 *
 * @tparam T
 * @param expr
 ...
 */
template <typename... T>
std::function<StaticFieldRef(T...)> GenerateFunction(
    const Expr* expr, const OutputSchema& input_schema, const OutputSchema* right_input_schema = nullptr,
    std::vector<const Expr*>* aggregate_expr_vec = nullptr,
    std::vector<std::function<void(AggregateIntermediateData&, StaticFieldRef)>>* aggregate_func_vec = nullptr) {
  if (expr->ret_type_ == RetType::STRING) {
    return __detail::GenerateStringExprFunction<T...>(expr, input_schema, right_input_schema, aggregate_expr_vec, aggregate_func_vec);
  } else if (expr->ret_type_ == RetType::INT) {
    return __detail::GenerateExprFunction<int64_t, T...>(expr, input_schema, right_input_schema, aggregate_expr_vec, aggregate_func_vec);
  } else if (expr->ret_type_ == RetType::FLOAT) {
    return __detail::GenerateExprFunction<double, T...>(expr, input_schema, right_input_schema, aggregate_expr_vec, aggregate_func_vec);
  } else {
    DB_ERR("Internal Error: Invalid Expr.");
  }
}
...
StaticFieldRef ExprFunction::Evaluate(InputTuplePtr input) const { return func_(input); }

ExprFunction::operator bool() const { return bool(func_); }

AggregateExprFunction::AggregateExprFunction(const Expr* expr, const OutputSchema& input_schema) {
  if (expr == nullptr) {
    func_ = nullptr;
  } else {
    ...
    func_ = GenerateFunction<InputTuplePtr, const AggregateIntermediateData*>(expr, input_schema_, nullptr, &aggregate_exprs, &aggregate_func_);
...
void AggregateExprFunction::FirstEvaluate(AggregateIntermediateData* aggregate_data, InputTuplePtr input) {
...
void AggregateExprFunction::Aggregate(AggregateIntermediateData* aggregate_data, InputTuplePtr input) {
  for (uint32_t id = 0; const auto& a : aggregate_exprfunction_) {
...
StaticFieldRef AggregateExprFunction::LastEvaluate(AggregateIntermediateData* aggregate_data, InputTuplePtr stored_parameter) {
...
class JoinInputTuplePtr {
 public:
  ...
  JoinInputTuplePtr(InputTuplePtr vec0, InputTuplePtr vec1) : vec0_(vec0), vec1_(vec1) {}
  template <typename T>
  T Read(uint32_t offset) const {
    return ~offset & 1 ? vec0_.Read<T>(offset >> 1) : vec1_.Read<T>(offset >> 1);
  }
  StaticFieldRef CreateStringRef(uint32_t offset) const {
    return ~offset & 1 ? vec0_.CreateStringRef(offset >> 1) : vec1_.CreateStringRef(offset >> 1);
  }
...
JoinExprFunction::JoinExprFunction(const Expr* expr, const OutputSchema& left_input_schema, const OutputSchema& right_input_schema) {
  if (expr == nullptr) {
    func_ = nullptr;
  } else {
    auto ch = GenerateFunction<JoinInputTuplePtr>(expr, left_input_schema, &right_input_schema);
    func_ = [=](InputTuplePtr x, InputTuplePtr y) { return ch(JoinInputTuplePtr(x, y)); };
  }
}

StaticFieldRef JoinExprFunction::Evaluate(InputTuplePtr input1, InputTuplePtr input2) const { return func_(input1, input2); }
