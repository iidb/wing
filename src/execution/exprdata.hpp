...
class InputTuplePtr {
 public:
  ...
  InputTuplePtr(const void* data)
    : data_(reinterpret_cast<const uint8_t*>(data)) {}
  InputTuplePtr(const std::vector<StaticFieldRef>& vec)
 ...
 * In join executor, the predicate receives two InputTuplePtr from two childern
 * and evaluate.
 */
class JoinExprFunction {
 public:
  JoinExprFunction(const Expr* expr, const OutputSchema& left_input_schema,
      const OutputSchema& right_input_schema);
  StaticFieldRef Evaluate(
      InputTuplePtr left_input, InputTuplePtr right_input) const;
  operator bool() const;
...
class AggregateExprFunction {
 public:
  ...
  void Aggregate(
      AggregateIntermediateData* aggregate_data, InputTuplePtr input);
  StaticFieldRef LastEvaluate(AggregateIntermediateData* aggregate_data,
      InputTuplePtr stored_parameter);
  operator bool() const;

 private:
  /* Function for last evaluate. */
  std::function<StaticFieldRef(InputTuplePtr, const AggregateIntermediateData*)>
      func_;
