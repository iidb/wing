...
class FilterExecutor : public Executor {
 public:
  FilterExecutor(const std::unique_ptr<Expr>& expr, const OutputSchema& input_schema, std::unique_ptr<Executor> ch)
  ...
  InputTuplePtr Next() override {
    ...
    while (ch_ret && (!predicate_ || predicate_.Evaluate(ch_ret).ReadInt() == 0)) ch_ret = ch_->Next();
    return ch_ret;
  }
