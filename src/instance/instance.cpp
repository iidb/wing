...
class Instance::Impl {
 public:
  Impl(std::string_view db_file, bool use_jit_flag) : use_jit_flag_(use_jit_flag), db_(db_file) {}
  void ExecuteShell() {
    auto& out = std::cout;
    auto& err = std::cerr;
    SQLCmdLine cmd;
    ...
    cmd.SetCommand("explain", [&](std::string_view statement) -> bool {
      ...
      err << fmt::format("Parsing completed in {} seconds.\n", watch.GetTimeInSeconds());
      watch.Reset();
      if (!ret.Valid()) {
    ...
    cmd.SetCommand("show", [&](std::string_view command) -> bool {
      uint32_t c = 0;
      while (c < command.size() && isspace(command[c])) c++;
    ...
    cmd.SetCommand("analyze", [&](std::string_view command) -> bool {
      ...
      if (command[c] == '\"') {
        c++;
        cend++;
        while (cend < command.size() && command[cend] != '\"') cend++;
        if (cend == command.size()) {
          out << "Invalid table name, expect '\"'." << std::endl;
          return true;
        }
      } else {
        while (cend < command.size() && (isalpha(command[cend]) || command[cend] == '_' || isdigit(command[cend]))) cend++;
      }
    ...
    cmd.SetCommand("stats", [&](std::string_view command) -> bool {
      ...
      if (stat == nullptr) {
        ...
        auto& tab = db_.GetDBSchema()[db_.GetDBSchema().Find(table_name).value()];
        out << "Tuple num: " << stat->GetTupleNum() << std::endl;
        for (uint32_t i = 0; i < (tab.GetHidePKFlag() ? tab.GetColumns().size() - 1 : tab.GetColumns().size()); i++) {
          out << fmt::format("Column {}: [Max: {}, Min: {}, Distinct rate: {}]\n", tab.GetColumns()[i].name_, stat->GetMax(i).ToString(),
      ...
      if (!ret.Valid()) {
        ...
        try {
          if (ret.GetPlan() == nullptr) {
            ...
            // Query
            auto [exe, use_jit] = GenerateExecutor(ret.GetPlan()->clone(), 0, ret.GetAST()->type_ == StatementType::SELECT);
            err << fmt::format("Generate executor in {} seconds.\n", watch.GetTimeInSeconds());
            auto output_schema = ret.GetPlan()->output_schema_;
            // Release unused memory
            ret.Clear();
            watch.Reset();
            auto result = GetResultFromExecutor(exe, use_jit, output_schema);
            err << fmt::format("Execute in {} seconds.\n", watch.GetTimeInSeconds());
    ...
    if (!ret.Valid()) {
      ...
      try {
        if (ret.GetPlan() == nullptr) {
          ...
          // Query
          auto [exe, use_jit] = GenerateExecutor(ret.GetPlan()->clone(), txn_id, ret.GetAST()->type_ == StatementType::SELECT);
          auto output_schema = ret.GetPlan()->output_schema_;
          // Release unused memory
          ret.Clear();
          auto result = GetResultFromExecutor(exe, use_jit, output_schema);
        ...
        DB_INFO("{}", e.what());
        return ResultSet("", fmt::format("DBException occurs. what(): {}\n", e.what()));
      }
    }
  }
 ...
 private:
  void CreateTable(const ParserResult& result, size_t txn_id) {
    ...
    if (db_.GetDBSchema().Find(a->table_name_)) {
      throw DBException("Create table \'{}\' error: table exists.", a->table_name_);
    }
    ...
    for (uint32_t i = 0; auto& col : a->columns_) {
      ...
      if (col.is_foreign_key_) {
        ...
        if (!ref) {
          throw DBException("Foreign key error: Referred table \'{}\' doesn't exist.", col.ref_table_name_);
        }
        // Referred key must be primary key.
        if (db_.GetDBSchema()[ref.value()].GetPrimaryKeySchema().name_ != col.ref_column_name_) {
          throw DBException("Foreign key error: Referred table \'{}\' has no primary key named \'{}\'", col.ref_table_name_, col.ref_column_name_);
        }
        if (db_.GetDBSchema()[ref.value()].GetPrimaryKeySchema().type_ != col.types_) {
          throw DBException("Foreign key error: Referred key \'{}\'.\'{}\' has a different type.", col.ref_table_name_, col.ref_column_name_);
        }
        if (db_.GetDBSchema()[ref.value()].GetPrimaryKeySchema().size_ != col.size_) {
          throw DBException("Foreign key error: Referred key \'{}\'.\'{}\' has a different size.", col.ref_table_name_, col.ref_column_name_);
        }
        fk.emplace_back(i, col.ref_table_name_, col.ref_column_name_, col.column_name_, col.types_, col.size_);
      }
      i++;
    }
    if (primary_key_index != (~0u)) {
      ...
      if (db_.GetDBSchema().Find(ref_table_name)) {
        throw DBException("Create ref table \'{}\' error: table exists.", ref_table_name);
      }
      std::vector<ColumnSchema> col2;
      col2.push_back(ColumnSchema{DB::GenRefColumnName(columns[primary_key_index].name_), FieldType::INT64, 8});
      col2.push_back(columns[primary_key_index]);
      auto col2_clone(col2);
      db_.CreateTable(TableSchema(std::move(ref_table_name), std::move(col2), std::move(col2_clone), 1, false, false, {}));
    } else {
      ...
      auto_gen_flag = true;
      columns.push_back(ColumnSchema{DB::GenDefaultPKName(), FieldType::INT64, 8});
    }
    auto storage_columns = columns;
    // Arrange the columns in storage.
    // In storage, VARCHAR strings are behind all other fixed fields.
    std::sort(storage_columns.begin(), storage_columns.end(), [&](const auto& x, const auto& y) {
      return (x.type_ == FieldType::CHAR || x.type_ == FieldType::VARCHAR) < (y.type_ == FieldType::CHAR || y.type_ == FieldType::VARCHAR);
    });
    db_.CreateTable(TableSchema(std::string(a->table_name_), std::move(columns), std::move(storage_columns), primary_key_index, auto_gen_flag,
                                hide_flag, std::move(fk)));
  }
  ...
  void DropTable(const ParserResult& result, size_t txn_id) {
    ...
    if (!index.has_value()) {
      throw DBException("Drop table error: table \'{}\' doesn't exist.", stmt->table_name_);
    }
    ...
    if (!tab.GetHidePKFlag()) {
      auto ret = parser_.Parse(fmt::format("select * from {} where {} > 0;", DB::GenRefTableName(stmt->table_name_),
      ...
      if (auto ret = exe.first->Next(); ret) {
        throw DBException(
            "Drop table error: exists reference to {}={}", tab.GetPrimaryKeySchema().name_,
            ret.Read<StaticFieldRef>(1 * sizeof(StaticFieldRef)).ToString(tab.GetPrimaryKeySchema().type_, tab.GetPrimaryKeySchema().size_));
      }
    }
    // If it has foreign key, then it must execute "delete from A" to decrement the refcounts.
    if (tab.GetFK().size() > 0) {
      auto ret = parser_.Parse(fmt::format("delete from {};", stmt->table_name_), db_.GetDBSchema());
      auto exe = GenerateExecutor(ret.GetPlan()->clone(), txn_id, false);
      GetResultFromExecutor(exe.first, exe.second, ret.GetPlan()->output_schema_);
    }
  ...
  // After this function returns, std::unique_ptr<Executor> should be released immediately!!
  // Because TupleStore in JitExecutor has been moved.
  TupleStore GetResultFromExecutor(std::unique_ptr<Executor>& exe, bool use_jit, const OutputSchema& output_schema) {
    if (use_jit) {
      exe->Init();
      auto result = const_cast<TupleStore*>(reinterpret_cast<const TupleStore*>(exe->Next().Data()));
   ...
   * For normal executors, the result is returned by returning a tuple at a time.
   */
  TupleStore GetTuplesFromNext(std::unique_ptr<Executor>& exe, const OutputSchema& schema) {
   ...
   */
  std::string FormatOutputTable(const TupleStore& output, const OutputSchema& schema, uint32_t limit = 100) {
    std::vector<uint8_t*> vec;
    if (output.GetPointerVec().size() > limit) {
      for (uint32_t i = 0; i < limit / 2; i++) {
        vec.push_back(output.GetPointerVec()[i]);
      }
      for (uint32_t i = output.GetPointerVec().size() - limit / 2; i < output.GetPointerVec().size(); i++) {
        vec.push_back(output.GetPointerVec()[i]);
      }
    } else {
    ...
    for (uint32_t i = 0; i < schema.Size(); i++) {
      ...
      for (auto& a : vec) {
        auto str = InputTuplePtr(a).Read<StaticFieldRef>(i * sizeof(StaticFieldRef)).ToString(schema[i].type_, schema[i].size_);
    ...
    for (uint32_t j = 0; j < schema.Size(); ++j) {
      ret += '|';
      ret += std::string(lengths[j] - schema[j].column_name_.length(), ' ') + schema[j].column_name_;
    }
    ret += "|\n";
    for (auto& L : lengths) {
      ret += '+' + std::string(L, '-');
    ...
    for (uint32_t i = 0; i < vec.size(); i++) {
      ...
      for (uint32_t j = 0; j < schema.Size(); j++) {
        auto str = InputTuplePtr(vec[i]).Read<StaticFieldRef>(j * sizeof(StaticFieldRef)).ToString(schema[j].type_, schema[i].size_);
...
Instance::Instance(std::string_view db_file, bool use_jit_flag) { ptr_ = std::make_unique<Impl>(db_file, use_jit_flag); }
Instance::~Instance() {}
ResultSet Instance::Execute(std::string_view statement) { return ptr_->Execute(statement); }
void Instance::ExecuteShell() { ptr_->ExecuteShell(); }
void Instance::Analyze(std::string_view table_name) { ptr_->Analyze(table_name); }
std::unique_ptr<PlanNode> Instance::GetPlan(std::string_view statement) { return ptr_->GetPlan(statement); }
