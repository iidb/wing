...
/**
 *
 * We parse the SQL statement and analysis the type of result (SQL is a strongly typed language).
 *
 * We also bind table name and column name to the corresponding ID in the database.
 *
 * But this data structure cannot be executed directly, we use this to generate execution plan.
 *
 */
 ...
 * ResultColumn.
 *
 * The result that SELECT statement returns. It can be either * (e.g. select * from A;) or <Expr>. (e.g. select A.a + 1, A.b from A;)
 *
 *                +- StarResultColumn: This means returns all columns.
 *                |
 * ResultColumn --+
 *                |
 *                +- ExprResultColumn: This column returns a expression containing column references and constants.
 *
 */
 ...
 * OrderByElement
 *
 * Whether the result that SELECT statement returns needs to be sorted and how to sort (ASC/DESC).
 *
 */
 ...
 * ResultTableType
 *
 * Which tables does SELECT statement select from? It can be normal tables (e.g. select * from A;)
 * or join-results (e.g. select * from A INNER JOIN B ON A.a = B.b;)
 * or sub-queries (e.g. select B.b from (select A.a + 1 as b from A) as B;)
 * or values (e.g. select * from (values (2, 3), (4, 5));, or insert into A values (2), (3);)
 *
 *            +- SubqueryTableRef: a select statement.
 *            |
 * TableRef --+- NormalTableRef: ID that denotes a column from table.
 *            |
 *            +- JoinTableRef: Join two tables or subqueries or other joins on some conditions.
 *            |           
 *            +- ValuesTableRef: An in-memory temporary table generated by the data in the statement.
 *
 *
 */
 ...
 * Statements.
 *
 * Select: select <ResultColumn-list> from <TableRef-list> where Expr group by <Expr-list> having <Expr> order by
 * <Order By Element-list> limit <Expr>;
