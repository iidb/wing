...
static const char* all_tokens[] = {"select",
                                   "from",
                                   "where",
                                   "insert",
                                   "into",
 ...
 public:
  ...
  int Search(std::string_view s) {
    ...
    for (auto& ch : s) {
      uint32_t a = static_cast<uint32_t>(ch);
      if (a > 128) break;
      if (std::isalpha(a)) a = std::tolower(a);
      if (!next_[a][rt]) break;
      rt = next_[a][rt];
    }
    if (!rt) return -1;
...
class SimpleTokenizer {
 public:
  SimpleTokenizer() {
    for (uint32_t i = 0; i < sizeof(all_tokens) / sizeof(char*); i++) all_tokens_and_op_trie.Insert(all_tokens[i], i);
  }
  ...
  std::pair<TokenType, std::string_view> Read() { return current_result_.second; }
  TokenType ReadType() { return Read().first; }
  std::string_view ReadStr() { return Read().second; }
  std::pair<uint32_t, std::string> CurrentPosition() { return {current_result_.first, _get_word(current_result_.second)}; }
 ...
 private:
  ...
  std::pair<TokenType, std::string_view> match_name() {
    ...
    while (new_index < str_.size() &&
           (std::isalpha(str_[new_index]) || std::isdigit(str_[new_index]) || str_[new_index] == '_' || (flag && str_[new_index] != '\"'))) {
      new_index++;
    }
    if (new_index < str_.size() && str_[new_index] == '\"') {
      new_index++;
    ...
    index_ = new_index;
    return {TokenType::_TABLENAME, str_.substr(old_index + flag, new_index - old_index - flag * 2)};
  }
  // Constants are like A or A.B or AeB or Ae-B or A. where A, B are non-empty digit strings, i.e. (0-9)+
  std::pair<TokenType, std::string_view> match_constant() {
    if (str_[index_] == '.' && (index_ >= str_.length() || !std::isdigit(str_[index_]))) return {TokenType::_LITERIAL_INVALID_NUMBER, {}};
    auto new_index = index_;
    bool dots = false, es = false;
    while (new_index < str_.size() && (std::isdigit(str_[new_index]) || str_[new_index] == '.' || str_[new_index] == 'e' || str_[new_index] == 'E')) {
      if ((es || dots) && str_[new_index] == '.') return {TokenType::_LITERIAL_INVALID_NUMBER, {}};
      dots |= str_[new_index] == '.';
      if (str_[new_index] == 'e' || str_[new_index] == 'E') {
        if (new_index + 1 < str_.size() && !es) {
          es = true;
          if (str_[new_index + 1] == '-' || str_[new_index + 1] == '+') new_index += 1;
        } else
          return {TokenType::_LITERIAL_INVALID_NUMBER, {}};
      }
      new_index++;
    }
    if (new_index < str_.size() && dots && str_[new_index] == '.') return {TokenType::_LITERIAL_INVALID_NUMBER, {}};
    if (new_index < str_.size() && (std::isalpha(str_[new_index]) || str_[new_index] == '_')) return {TokenType::_LITERIAL_INVALID_NUMBER, {}};
    auto old_index = index_;
    index_ = new_index;
  ...
  // For convenience, when it reads '\"', it will recognize it as '\' and '"'. It will only skip '"', but it will not delete '\'.
  std::pair<TokenType, std::string_view> match_string() {
    auto new_index = index_;
    new_index++;
    while (new_index < str_.size() && str_[new_index] != '\'') new_index += str_[new_index] == '\\' ? 2 : 1;
    if (new_index >= str_.size() || str_[new_index] != '\'') return {TokenType::_LITERIAL_INVALID_STRING, {}};
    new_index++;
    auto old_index = index_;
    index_ = new_index;
    return {TokenType::_LITERIAL_STRING, str_.substr(old_index + 1, new_index - old_index - 2)};
  }

  std::pair<TokenType, std::string_view> _read() {
    if (!_eliminate_space()) return {TokenType::_END, {}};
    // if the SQL command uses a reserved word, then it cannot write a-zA-z_0-9 behind it.
    auto rt = all_tokens_and_op_trie.Search(str_.substr(index_, str_.length() - index_));
    if (rt != -1) {
      auto len = std::strlen(all_tokens[rt]);
      if (rt >= static_cast<int32_t>(TokenType::_STAR) &&
          !(rt <= static_cast<int32_t>(TokenType::_OR) && rt >= static_cast<int32_t>(TokenType::_AND))) {
        index_ += len;
        if (rt >= static_cast<int32_t>(TokenType::_OPERATOR)) return {TokenType::_OPERATOR, str_.substr(index_ - len, len)};
        return {static_cast<TokenType>(rt), str_.substr(index_ - len, len)};
      }
      if (len + index_ == str_.length() || (!std::isdigit(str_[len + index_]) && !std::isalpha(str_[len + index_]) && str_[len + index_] != '_')) {
        index_ += len;
        if (rt >= static_cast<int32_t>(TokenType::_AND)) return {TokenType::_OPERATOR, str_.substr(index_ - len, len)};
        return {static_cast<TokenType>(rt), str_.substr(index_ - len, len)};
      }
    }
    ...
    if (str_[index_] == '.' || isdigit(str_[index_]))
      ...
      return match_string();
    else if (isalpha(str_[index_]) || str_[index_] == '_' || str_[index_] == '\"')
      return match_name();
  ...
  bool _eliminate_space() {
    while (index_ < str_.size() && std::isspace(str_[index_])) index_++;
    return index_ < str_.size();
  }
  std::string_view str_;
  uint32_t index_;
  // The cost of vector push_back is very high. For inserting 1e6 tuples of 3 fields, it takes 0.2 seconds out of 0.7 seconds.
  // std::vector<std::pair<uint32_t, std::pair<TokenType, std::string_view>>> result_;
...
class Parser::Impl {
 ...
 public:
  std::pair<std::unique_ptr<Statement>, std::string> Parse(std::string_view statement) {
    ...
    try {
      ...
      return {nullptr, fmt::format("Syntax error at {}, near \'{}\', error message: {}", data.first, data.second, e.what())};
    } catch (...) {
      auto data = reader_.CurrentPosition();
      return {nullptr, fmt::format("Syntax error at {}, near \'{}\'", data.first, data.second)};
    }
  }
 ...
 private:
  ...
  std::unique_ptr<Statement> parse() {
    ...
    throw ParserException("Expect \'select\' or \'update\' or \'drop\' or \'create\' or \'insert\'");
  }

  std::unique_ptr<SelectStatement> select_clause() {
    auto ret = select_or_subquery_clause();
    if (reader_.ReadType() != TokenType::_SEMICOLON) throw ParserException("Expect \';\'");
    reader_.Next();
    return ret;
  }
  ...
  std::unique_ptr<SelectStatement> select_or_subquery_clause() {
    ...
    ret->result_column_ = _get_list_of<std::unique_ptr<ResultColumn>>([this]() { return result_column_clause(); });

    if (reader_.ReadType() == TokenType::_SEMICOLON)
      return ret;
    else if (reader_.ReadType() == TokenType::FROM) {
      reader_.Next();
      ret->tables_ = _get_list_of<std::unique_ptr<TableRef>>([this]() { return table_clause(); });
    }
    ...
    if (reader_.ReadType() == TokenType::_SEMICOLON)
      ...
      ret->group_by_ = _get_list_of<std::unique_ptr<Expr>>([this]() { return expr_clause(); });
    ...
    if (reader_.ReadType() == TokenType::_SEMICOLON)
      ...
      reader_.Next();
      _match_token("by", TokenType::BY);
      ret->order_by_ = _get_list_of<std::unique_ptr<OrderByElement>>([this]() { return order_by_clause(); });
    }
  ...
  std::unique_ptr<CreateIndexStatement> create_index_clause() {
    ...
    ret->indexed_column_names_ = _get_list_of<std::string>([this]() { return table_name_clause(); });
  ...
  // update A set A.a = A.a + B.b from (select sum(C.c) as b, C.id from C group by 2) as B where C.id = A.id
  ...
  // output rows is used for updating the target table. The output row selected is arbitrary and might change from one release of SQLite to the next,
  // or from one run to the next.
  std::unique_ptr<UpdateStatement> update_clause() {
    ...
    ret->updates_ = _get_list_of<std::unique_ptr<ColumnUpdate>>([this]() { return set_clause(); });
    if (reader_.ReadType() == TokenType::_SEMICOLON)
      return ret;
    else if (reader_.ReadType() == TokenType::FROM) {
      reader_.Next();
      ret->other_tables_ = _get_list_of<std::unique_ptr<TableRef>>([this]() { return table_clause(); });
    }
    if (reader_.ReadType() == TokenType::_SEMICOLON)
      return ret;
    else if (reader_.ReadType() == TokenType::WHERE) {
  ...
  std::unique_ptr<OrderByElement> order_by_clause() {
    ...
    ret->expr_ = expr_clause();
    auto [type, str] = reader_.Read();
    if (type != TokenType::ASC && type != TokenType::DESC) throw ParserException("Expect \'asc\' or \'desc\'.");
    reader_.Next();
  ...
  std::unique_ptr<TableAs> maybe_as_clause() {
    if (reader_.ReadType() == TokenType::_TABLENAME || reader_.ReadType() == TokenType::AS) {
      if (reader_.ReadType() == TokenType::AS) {
        reader_.Next();
      }
      auto ret = std::make_unique<TableAs>();
      ret->table_name_ = table_name_clause();
      if (reader_.ReadType() != TokenType::_LEFTQ) return ret;
      _match_token("(", TokenType::_LEFTQ);
      ret->column_names_ = _get_list_of<std::string>([this]() { return table_name_clause(); });
      _match_token(")", TokenType::_RIGHTQ);
      return ret;
    } else {
      return nullptr;
  ...
  std::unique_ptr<TableRef> table_clause_with_level(uint32_t level) {
    ...
    // This is A join B on condition(A, B) join C on condition(AB, C) join D on condition (ABC, D)...
    // where A, B, C, D are tables or subqueries or bracketed expressions.
    // subqueries need to have alias.
    while (true) {
      auto [type, str] = reader_.Read();
      if (type != TokenType::JOIN && type != TokenType::INNER) break;
      reader_.Next();
      if (type == TokenType::INNER) _match_token("join", TokenType::JOIN);
  ...
  std::unique_ptr<TableRef> table_clause() { return table_clause_with_level(1); }
    ...
    while (true) {
      ...
      if (type == TokenType::_TABLENAME) {
        ...
        if (reader_.ReadType() == TokenType::AUTOINC) {
          reader_.Next();
          if (has_auto_inc) {
            throw ParserException("Table must have at most one auto_increment key.");
          } else {
            has_auto_inc = true;
          }
          if (col.types_ != FieldType::INT32 && col.types_ != FieldType::INT64) {
        ...
        if (reader_.ReadType() == TokenType::PRIMARY) {
          ...
          if (has_primary_key) {
            throw ParserException("A table cannot have two or more primary keys.");
          } else {
        ...
        if (reader_.ReadType() != TokenType::_COMMA) break;
        reader_.Next();
        continue;
      }
    ...
    for (uint32_t i = 0; i < ret.size(); i++) {
      for (uint32_t j = 0; j < i; ++j)
        if (ret[i].column_name_ == ret[j].column_name_) {
          throw ParserException(fmt::format("Duplicate column name \'{}\'.", ret[i].column_name_));
        }
    }
    return ret;
  }
    ...
    while (true) {
      ...
      auto values = _get_list_of<Field>([this]() {
        ...
        if (expr->type_ == ExprType::LITERAL_FLOAT) {
          return Field::CreateFloat(FieldType::FLOAT64, 8, static_cast<const LiteralFloatExpr*>(expr.get())->literal_value_);
        } else if (expr->type_ == ExprType::LITERAL_INTEGER) {
          return Field::CreateInt(FieldType::INT64, 8, static_cast<const LiteralIntegerExpr*>(expr.get())->literal_value_);
        } else if (expr->type_ == ExprType::LITERAL_STRING) {
          return Field::CreateString(FieldType::CHAR, static_cast<const LiteralStringExpr*>(expr.get())->literal_value_);
        } else {
          throw ParserException("Expression in the VALUES clause is not constant.");
        }
      });
      if (first_flag) {
      ...
      ret->values_.insert(ret->values_.end(), std::make_move_iterator(values.begin()), std::make_move_iterator(values.end()));
  ...
  std::unique_ptr<ColumnUpdate> set_clause() {
    ...
    if (type == TokenType::_TABLENAME) {
      ...
      if (type2 != TokenType::_DOT) {
        ...
        // expect '='
        if (type2 != TokenType::_OPERATOR || str2 != "=") throw ParserException("Expect \'=\'.");
        reader_.Next();
      ...
      // expect column_name
      if (type3 != TokenType::_TABLENAME) throw ParserException("Invalid column name.");
      reader_.Next();
      ret->table_name_ = std::string(str);
      ret->column_name_ = std::string(str3);
      auto [type4, str4] = reader_.Read();
      // expect '='
      if (type4 != TokenType::_OPERATOR || str4 != "=") throw ParserException("Expect \'=\'.");
      reader_.Next();
      ret->update_value_ = expr_clause();
      return ret;
    }
  ...
  bool is_literal(std::unique_ptr<Expr>& expr) {
    return expr->type_ == ExprType::LITERAL_FLOAT || expr->type_ == ExprType::LITERAL_INTEGER || expr->type_ == ExprType::LITERAL_STRING;
  }

  // If all operands are literal exprs then the expr can also be converted to literal expr.
  std::unique_ptr<Expr> maybe_get_literal_expr(std::unique_ptr<Expr>& ch0, std::unique_ptr<Expr>& ch1, OpType op) {
    if (op == OpType::BITAND || op == OpType::BITLSH || op == OpType::BITRSH || op == OpType::BITOR || op == OpType::BITXOR || op == OpType::MOD ||
        op == OpType::AND || op == OpType::OR) {
      if (ch0->type_ != ExprType::LITERAL_INTEGER || ch1->type_ != ExprType::LITERAL_INTEGER) {
        throw ParserException(fmt::format("Operator (OpType={}) only used between integers.", (uint32_t)op));
      }
    }
  ...
  if (op == OpType::expect_op) {                        \
    ...
    if (ch0->type_ != ch1->type_) {
      if (ch0->type_ == ExprType::LITERAL_STRING) {
        throw ParserException("Invalid operators between STRING and other type.");
      }
      double v0, v1;
...
#undef GEN_FUNC
  ...
  void maybe_constant_fold(std::unique_ptr<Expr>& expr) {
    if (expr->type_ == ExprType::BINCONDOP) {
      ...
      if (expr->ch0_->type_ == ExprType::LITERAL_STRING) {
        ...
        auto v0 = static_cast<LiteralIntegerExpr*>(expr->ch0_.get())->literal_value_;
      ...
      if (expr->ch0_->type_ == ExprType::LITERAL_STRING) {
        ...
        auto v0 = static_cast<LiteralFloatExpr*>(expr->ch0_.get())->literal_value_;
  ...
  // F0 := (Expr) | Table.Column | "String" | Number | -F0 | not F0 | +F0 | aggregate_function(Expr)
  // F1 := F0 * F1 | F0 / F1 | F0 % F1 | F0
  // F2 := F1 + F2 | F1 - F2 | F1
  // F3 := F2 ^ F3 | F2 & F3 | F2 | F3 | F2 << F3 | F2 >> F3
  ...
  std::pair<std::unique_ptr<Expr>, bool> expr_clause_with_level(uint32_t level) {
    if (level == 0) {
      auto [type, str] = reader_.Read();
      // F0 := Table.Column
      if (type == TokenType::_TABLENAME) {
        ...
        auto ret = std::make_unique<ColumnExpr>(std::string(str), std::string(str3));
        return {std::move(ret), 0};
      ...
      } else if (type == TokenType::_OPERATOR && (str == "-" || str == "not" || str == "+")) {
        // F0 := -F0 | not F0 | +F0
        reader_.Next();
        // Check whether it is negative number
        if (str == "-") {
          auto [type, str] = reader_.Read();
          if (type >= TokenType::_LITERIAL_INTEGER && type < TokenType::_LITERIAL_INVALID_NUMBER) {
            if (type == TokenType::_LITERIAL_INTEGER || type == TokenType::_LITERIAL_INTEGER_OVERFLOW) {
              try {
                auto ret = std::make_unique<LiteralIntegerExpr>(StringToInt(str, -1));
                reader_.Next();
                return {std::move(ret), 0};
              } catch (std::invalid_argument const& ex) {
              ...
              try {
                auto ret = std::make_unique<LiteralFloatExpr>(StringToDouble(str, -1));
                reader_.Next();
                return {std::move(ret), 0};
              } catch (std::invalid_argument const& ex) {
                throw ParserException("Invalid number.");
        ...
        if (str == "+") {
          if (expr->type_ != ExprType::BINCONDOP && expr->type_ != ExprType::UNARYCONDOP)
            return {std::move(expr), 0};
          else
            throw ParserException("Invalid operand of type BOOLEAN to unary operator \'+\'.");
        } else if (str == "-") {
          if (expr->type_ != ExprType::BINCONDOP && expr->type_ != ExprType::UNARYCONDOP)
            return {std::make_unique<UnaryExpr>(OpType::NEG, std::move(expr)), 0};
          else
            throw ParserException("Invalid operand of type BOOLEAN to unary operator \'-\'.");
        } else {
          if (expr->type_ != ExprType::BINCONDOP && expr->type_ != ExprType::UNARYCONDOP && expr->type_ != ExprType::LITERAL_INTEGER)
            throw ParserException("Operator NOT requires operands of type BOOLEAN.");
          return {std::make_unique<UnaryConditionExpr>(OpType::NOT, std::move(expr)), 0};
        }
      } else if (type == TokenType::MAX || type == TokenType::MIN || type == TokenType::SUM || type == TokenType::AVG || type == TokenType::COUNT) {
        // F0 := aggregate_function(Expr)
        reader_.Next();
        _match_token("(", TokenType::_LEFTQ);
        // count(*)
        if (type == TokenType::COUNT && reader_.ReadType() == TokenType::_STAR) {
          reader_.Next();
          _match_token(")", TokenType::_RIGHTQ);
          return {std::make_unique<AggregateFunctionExpr>(std::string(str), std::make_unique<LiteralIntegerExpr>(1)), 0};
        }
        auto bracketed_expr = expr_clause();
        maybe_constant_fold(bracketed_expr);
        _match_token(")", TokenType::_RIGHTQ);
        return {std::make_unique<AggregateFunctionExpr>(std::string(str), std::move(bracketed_expr)), 0};
      } else {
        _deal_with_number_exception(type);
      }
    ...
    // Multiplication is not communicate with division. For example: 2 * 5 / 10, if we first calculate 5 / 10,
    // Then it turns to 0 since it is integer division.
    while (true) {
      auto [type, str] = reader_.Read();
      if (type == TokenType::_OPERATOR || type == TokenType::_STAR) {
        ...
        // flag means this is a valid operator. If it is not operator, then we return expr directly.
        // flag_cond means this is a condition operator.
        // flag_logic_cond menas this is a logical operator.
        if (level == 1) {
          if (type == TokenType::_STAR || str == "*" || str == "/" || str == "%") {
        ...
        if (flag) {
          reader_.Next();
          auto [right_expr, is_right_bracketed] = expr_clause_with_level(level - 1);
          // Logical operators only accept operands that return boolean, i.e. expressions of condition operators.
          if ((left_expr->type_ == ExprType::BINCONDOP || left_expr->type_ == ExprType::UNARYCONDOP ||
               left_expr->type_ == ExprType::LITERAL_INTEGER) &&
              (right_expr->type_ == ExprType::BINCONDOP || right_expr->type_ == ExprType::UNARYCONDOP ||
               right_expr->type_ == ExprType::LITERAL_INTEGER) &&
              flag_logic_cond) {
            left_expr = std::make_unique<BinaryConditionExpr>(op_type, std::move(left_expr), std::move(right_expr));
            is_left_bracketed = 0;
          } else if (flag_logic_cond) {
            throw ParserException(fmt::format("Invalid operand of type to binary operator \'{}\'.", str));
          } else if (flag_cond) {
            left_expr = std::make_unique<BinaryConditionExpr>(op_type, std::move(left_expr), std::move(right_expr));
            is_left_bracketed = 0;
          } else {
            left_expr = std::make_unique<BinaryExpr>(op_type, std::move(left_expr), std::move(right_expr));
            is_left_bracketed = 0;
          }
          maybe_constant_fold(left_expr);
        } else
  ...
  std::unique_ptr<Expr> expr_clause() { return expr_clause_with_level(9).first; }
  ...
  std::vector<T> _get_list_of(F&& func) {
    ...
    while (true) {
      ret.push_back(func());
      auto [type, str] = reader_.Read();
      if (type != TokenType::_COMMA) break;
      reader_.Next();
    }
    return ret;
  }
  ...
  void _deal_with_number_exception(TokenType type) const {
    if (type == TokenType::_LITERIAL_FLOAT_OVERFLOW)
      ...
      throw ParserException("Invalid number.");
    else if (type == TokenType::_LITERIAL_INVALID_STRING || type == TokenType::_LITERIAL_STRING)
  ...
  void _match_token(std::string_view word, TokenType token) {
    if (reader_.ReadType() != token) throw ParserException(fmt::format("Expect \'{}\'.", word));
    reader_.Next();
  }
};
...
ParserResult Parser::Parse(std::string_view str, const DBSchema& schema) {
  ...
  if (errmsg != "") return {std::move(errmsg)};

  if (statement->type_ == StatementType::CREATE_INDEX || statement->type_ == StatementType::CREATE_TABLE ||
      statement->type_ == StatementType::DROP_INDEX || statement->type_ == StatementType::DROP_TABLE) {
    return {std::move(statement), nullptr, ""};
  }

  BasicPlanGenerator planner(schema);
  auto [plan, errmsg2] = planner.Plan(statement.get());
  if (errmsg2 != "") return {std::move(errmsg2)};
...
std::string SetToString(Ta&& first, const std::vector<Tb>& second, T&&... args)
  requires IsString<Ta> && fmt::is_formattable<Tb>::value;
template <typename Ta, typename Tb, typename... T>
std::string SetToString(Ta&& first, const std::unique_ptr<Tb>& second, T&&... args)
  ...
  requires IsString<Ta> && fmt::is_formattable<Tb>::value
{
  auto func = [](auto&& first, auto&& second, bool is_last) {
    return is_last ? fmt::format("{}: {}", first, second) : fmt::format("{}: {}, ", first, second);
  };
  if constexpr (sizeof...(args) > 0)
    return func(std::forward<Ta>(first), std::forward<Tb>(second), false) + impl::SetToString(std::forward<T>(args)...);
  else
  ...
  requires IsString<Ta> && CanPointerToString<Tb>
{
  ...
  for (uint32_t i = 0; i < second.size(); i++) ret += fmt::format("{}{}", second[i]->ToString(), i + 1 < second.size() ? ", " : "");
  ret += " }";
  if constexpr (sizeof...(args) > 0)
...
{
  std::string ret = fmt::format("{}: {{ ", first);
  for (uint32_t i = 0; i < second.size(); i++) ret += fmt::format("{}{}", second[i], i + 1 < second.size() ? ", " : "");
  ret += " }";
  if constexpr (sizeof...(args) > 0)
    return ret + ", " + impl::SetToString(std::forward<T>(args)...);
  else
...
std::string SetToString(Ta&& first, const std::unique_ptr<Tb>& second, T&&... args)
  requires IsString<Ta>
{
  return impl::SetToString(std::forward<Ta>(first), second == nullptr ? "" : second->ToString(), std::forward<T>(args)...);
}
...
#define ref_vec(T, unique_ptr_vec)                         \
  ({                                                       \
    std::vector<T*> ret;                                   \
    for (auto& a : unique_ptr_vec) ret.push_back(a.get()); \
    ret;                                                   \
  })

static const char* op_str[] = {"+", "-", "*", "/", "%", "&", "^", "|", "<<", ">>", "<", ">", "<=", ">=", "=", "<>", "and", "or", "not", "-"};

static const char* return_type_str[] = {"int", "float", "string", "null"};

static const char* field_type_str[] = {"int32", "int64", "float64", "char", "varchar", "empty"};

std::string BinaryExpr::ToString() const { return fmt::format("({}){}({})", ch0_->ToString(), op_str[static_cast<uint32_t>(op_)], ch1_->ToString()); }

std::string BinaryConditionExpr::ToString() const {
  return fmt::format("({}){}({})", ch0_->ToString(), op_str[static_cast<uint32_t>(op_)], ch1_->ToString());
}

std::string UnaryExpr::ToString() const { return fmt::format("{}({})", op_str[static_cast<uint32_t>(op_)], ch0_->ToString()); }

std::string UnaryConditionExpr::ToString() const { return fmt::format("{}({})", op_str[static_cast<uint32_t>(op_)], ch0_->ToString()); }

std::string LiteralStringExpr::ToString() const { return fmt::format("\"{}\"%string", literal_value_); }

std::string LiteralIntegerExpr::ToString() const { return fmt::format("{}%int", literal_value_); }

std::string LiteralFloatExpr::ToString() const { return fmt::format("{}%float", literal_value_); }

std::string ColumnExpr::ToString() const {
  if (table_name_ == "") {
    return fmt::format("{}%{}%{}", column_name_, id_in_column_name_table_, return_type_str[static_cast<uint32_t>(ret_type_)]);
  }
  return fmt::format("{}.{}%{}%{}", table_name_, column_name_, id_in_column_name_table_, return_type_str[static_cast<uint32_t>(ret_type_)]);
}

std::string CastExpr::ToString() const { return fmt::format("{}({})", return_type_str[static_cast<uint32_t>(ret_type_)], ch0_->ToString()); }

std::string AggregateFunctionExpr::ToString() const { return fmt::format("{}({})", func_name_, ch0_->ToString()); }

std::string ExprResultColumn::ToString() const { return SetToString("expr result column", expr_, "as", as_); }

std::string StarResultColumn::ToString() const { return "{ star result column }"; }

std::string OrderByElement::ToString() const { return SetToString("order by", expr_, "is asc", is_asc_); }

std::string TableAs::ToString() const { return SetToString("table name", table_name_, "column names", column_names_); }

std::string NormalTableRef::ToString() const { return SetToString("normal tableref", table_name_, "alias", as_); }

std::string JoinTableRef::ToString() const {
  return SetToString("join tableref 0", ch_[0], "join table 1", ch_[1], "predicate", predicate_, "as", as_);
}

std::string SubqueryTableRef::ToString() const { return SetToString("subquery tableref", ch_, "as", as_); }

std::string SelectStatement::ToString() const {
  return "select: " + SetToString("tables", ref_vec(TableRef, tables_), "result_column", ref_vec(ResultColumn, result_column_), "where", predicate_,
                                  "group by", ref_vec(Expr, group_by_), "having", having_, "order by", ref_vec(OrderByElement, order_by_),
                                  "limit count", limit_count_, "distinct", is_distinct_);
}

std::string ColumnDescription::ToString() const { return SetToString("column_name", column_name_, "field_type", static_cast<uint32_t>(types_)); }

std::string CreateTableStatement::ToString() const {
  return "create table: " + SetToString("table name", table_name_, "column descriptions", columns_);
}

std::string ColumnUpdate::ToString() const {
  return SetToString("table_name", table_name_, "column_name", column_name_, "update_value", update_value_);
}

std::string UpdateStatement::ToString() const {
  return "update: " + SetToString("table name", table_name_, "column updates", ref_vec(ColumnUpdate, updates_), "predicate", predicate_);
}

std::string InsertStatement::ToString() const { return "insert: " + SetToString("table name", table_name_, "insert data", insert_data_); }

std::string DropTableStatement::ToString() const { return "drop table: " + SetToString("table name", table_name_); }

std::string ValuesTableRef::ToString() const {
  return SetToString("values", values_, "number of fields per tuple", num_fields_per_tuple_, "as", as_);
}

std::string CreateIndexStatement::ToString() const { return "create index: " + SetToString("table name", table_name_, "index name", index_name_); }

std::string DropIndexStatement::ToString() const { return "drop index: " + SetToString("index name", index_name_); }

std::string DeleteStatement::ToString() const { return "delete: " + SetToString("table name", table_name_, "predicate", predicate_); }
...
std::unique_ptr<Expr> BinaryConditionExpr::clone() const {
  auto ret = std::make_unique<BinaryConditionExpr>(op_, ch0_->clone(), ch1_->clone());
  ret->ret_type_ = ret_type_;
  return ret;
}
