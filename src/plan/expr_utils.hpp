 ...
 public:
  ...
  static void DivideIntoPredicateList(
    ...
    if (expr->type_ == ExprType::BINCONDOP &&
        static_cast<BinaryConditionExpr*>(expr.get())->op_ == OpType::AND) {
  ...
  static std::vector<std::unique_ptr<Expr>> DivideIntoPredicateList(
  ...
  static void GetExprIds(const Expr* expr, std::vector<uint32_t>& result) {
    if (expr->type_ == ExprType::COLUMN) {
      result.push_back(
          static_cast<const ColumnExpr*>(expr)->id_table_in_planner_);
    } else {
      if (expr->ch0_ != nullptr)
        GetExprIds(expr->ch0_.get(), result);
      if (expr->ch1_ != nullptr)
        GetExprIds(expr->ch1_.get(), result);
    }
  }
  ...
  static std::unique_ptr<Expr> ApplyExprOnExpr(const Expr* expr,
    ...
    if (expr->type_ == ExprType::BINCONDOP || expr->type_ == ExprType::BINOP) {
      ...
      if (expr->type_ == ExprType::BINOP) {
        ret = std::make_unique<BinaryExpr>(
            static_cast<const BinaryExpr*>(expr)->op_, std::move(L),
            std::move(R));
      } else {
        ret = std::make_unique<BinaryConditionExpr>(
            static_cast<const BinaryConditionExpr*>(expr)->op_, std::move(L),
            std::move(R));
      }
      ...
      return ret;
               ...
               expr->type_ == ExprType::CAST) {
      auto L = ApplyExprOnExpr(expr->ch0_.get(), input_exprs, input_schema);
      std::unique_ptr<Expr> ret;
      if (expr->type_ == ExprType::UNARYOP) {
        ret = std::make_unique<UnaryExpr>(
            static_cast<const UnaryExpr*>(expr)->op_, std::move(L));
      } else if (expr->type_ == ExprType::UNARYCONDOP) {
        ret = std::make_unique<UnaryConditionExpr>(
            static_cast<const UnaryConditionExpr*>(expr)->op_, std::move(L));
      ...
      auto L = ApplyExprOnExpr(expr->ch0_.get(), input_exprs, input_schema);
      auto ret = std::make_unique<AggregateFunctionExpr>(
          static_cast<const AggregateFunctionExpr*>(expr)->func_name_,
          std::move(L));
      ret->ret_type_ = expr->ret_type_;
      return ret;
    } else if (expr->type_ == ExprType::COLUMN) {
      auto i = input_schema.FindById(
          static_cast<const ColumnExpr*>(expr)->id_in_column_name_table_);
      // i should have value.
