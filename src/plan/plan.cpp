...
class BasicPlanGenerator::Impl {
 ...
 private:
  // get_value_id get the index of (table_name, column_name) in column_name_table
  // column_name_table_index is set of indexes that are valid in current scope.
  // For example, in the following SQL statement:
  //      select a, c from (select a as b from A) as D, (select b as c from A) as D
  // column name 'b' is valid in both subqueries, i.e., b in the second subquery doesn't refer to b in the first subquery.
  uint32_t get_value_id(const auto& table_name, const auto& column_name, const OutputSchema& column_name_table) {
    uint32_t ret = ~0u;
    for (uint32_t index = 0; const auto& temp_column : column_name_table.GetCols()) {
      ...
      if ((tab == table_name || table_name == "") && column_name == col) {
        if (~ret) {
          if (table_name != "")
            throw PlannerException(fmt::format("Column \'{}.{}\' is ambigorous.", table_name, column_name));
          else
            throw PlannerException(fmt::format("Column \'{}\' is ambigorous: \'{}.{}\', \'{}.{}\'.", column_name, column_name_table[ret].table_name_,
                                               column_name, tab, column_name));
        }
    ...
    if (ret == (~0u)) {
      if (table_name != "")
        throw PlannerException(fmt::format("Column \'{}.{}\' is undefined.", table_name, column_name));
      else
        throw PlannerException(fmt::format("Column \'{}\' is undefined.", column_name));
    }
    return ret;
  }
  // analysis_expr analyses the return type and find the index of (table_name, column_name) using column_name_table and column_name_table_index
  // It also calculates the number of aggregation functions.
  // It checks: (1) whether this is a constant expression. (2) whether it has nested aggregation functions.
  // (3) whether there exists invalid usage of operands.
  // If the types of operands are different, it will try to cast one of them to higher type.
  // For example, if the expression is 1 (INT) + 1.1 (FLOAT).
  // Then it will cast 1 (INT) to FLOAT and add a CastExpr to the Expr.
  AnalysisExprResult analysis_expr(Expr* expr, const OutputSchema& column_name_table) {
    ...
    if (expr->type_ == ExprType::LITERAL_FLOAT) {
      ...
      result.aggregate_expr_num = retl.aggregate_expr_num + retr.aggregate_expr_num;
      auto ltype = this_expr->ch0_->ret_type_;
      auto rtype = this_expr->ch1_->ret_type_;
      if (ltype != rtype) {
        if (ltype == RetType::STRING || rtype == RetType::STRING)
          throw PlannerException("Arithmetic operators between STRINGs and other type are invalid.");
        if (ltype == RetType::FLOAT) {
          this_expr->ch1_ = std::make_unique<CastExpr>(std::move(this_expr->ch1_));
          this_expr->ch1_->ret_type_ = RetType::FLOAT;
          this_expr->ret_type_ = RetType::FLOAT;
        } else {
          this_expr->ch0_ = std::make_unique<CastExpr>(std::move(this_expr->ch0_));
          this_expr->ch0_->ret_type_ = RetType::FLOAT;
          this_expr->ret_type_ = RetType::FLOAT;
        }
      } else {
        if (ltype == RetType::STRING) {
          throw PlannerException("Arithmetic operators between STRINGs are invalid.");
        } else {
          this_expr->ret_type_ = ltype;
        }
      }
      if (this_expr->ret_type_ == RetType::FLOAT) {
        if (this_expr->op_ == OpType::BITAND || this_expr->op_ == OpType::BITOR || this_expr->op_ == OpType::BITRSH ||
            this_expr->op_ == OpType::BITLSH || this_expr->op_ == OpType::BITXOR || this_expr->op_ == OpType::MOD) {
      ...
      if (ltype != rtype) {
        if (ltype == RetType::STRING || rtype == RetType::STRING)
          throw PlannerException("Relational operator between STRING and other type is invalid.");
        if (ltype == RetType::FLOAT) {
          this_expr->ch1_ = std::make_unique<CastExpr>(std::move(this_expr->ch1_));
          this_expr->ch1_->ret_type_ = RetType::FLOAT;
        } else {
        ...
        if (ltype == RetType::STRING) {
          if (this_expr->op_ != OpType::LT && this_expr->op_ != OpType::GT && this_expr->op_ != OpType::LEQ && this_expr->op_ != OpType::GEQ &&
              this_expr->op_ != OpType::EQ && this_expr->op_ != OpType::NEQ) {
            throw PlannerException("Relational operator between STRINGs is invalid.");
          }
        }
      }
      ...
      result = ret;
      auto ltype = this_expr->ch0_->ret_type_;
      if (ltype == RetType::STRING) throw PlannerException("Unary conditional operator on STRING is invalid.");
      ...
      result = ret;
      auto ltype = this_expr->ch0_->ret_type_;
      if (ltype == RetType::STRING) throw PlannerException("Unary arithmetic operator on STRING is invalid.");
      expr->ret_type_ = ltype;
    } else if (expr->type_ == ExprType::COLUMN) {
      auto this_expr = static_cast<ColumnExpr*>(expr);
      auto id = get_value_id(this_expr->table_name_, this_expr->column_name_, column_name_table);
      auto type = column_name_table[id].type_;
      this_expr->id_in_column_name_table_ = column_name_table[id].id_;
      // Get table_id
      this_expr->id_table_in_planner_ = table_id_table_[this_expr->id_in_column_name_table_];
  ...
  // (1) insert into A values ... This inserts tuples containing constant fields.
  // (2) insert into A select ... This inserts the result of the subquery, we use plan_select to plan the subquery.
  // and store it as the child plan node.
  std::unique_ptr<PlanNode> plan_insert(InsertStatement* statement) {
    ...
    if (!table_id.has_value()) {
      throw PlannerException(fmt::format("Table \'{}\' does not exist.", statement->table_name_));
    }
    ...
    if (insert_data->output_schema_.Size() != (table_schema.GetHidePKFlag() ? table_columns.size() - 1 : table_columns.size())) {
    ...
    for (uint32_t index = 0; auto& a : insert_data->output_schema_.GetCols()) {
      if (!IsTypeEqual(a.type_, table_columns[index].type_)) {
        throw PlannerException(fmt::format("The type of the {}-th field in insert value is not correct.", index + 1));
      }
      index += 1;
    }
    ...
    ret->output_schema_.Append(OutputColumnData{column_id_++, "", "inserted rows", FieldType::INT64, 0});
  ...
  std::unique_ptr<PlanNode> plan_update(UpdateStatement* statement) {
    ...
    for (auto& a : statement->other_tables_) {
      read_from_table = join_two_plan(std::move(read_from_table), plan_table(a.get()));
    }

    for (auto& a : statement->updates_) {
      auto expr_result = analysis_expr(a->update_value_.get(), read_from_table->output_schema_);
      if (expr_result.aggregate_expr_num > 0) {
        throw PlannerException("We do not support aggregate functions in update statements.");
      }
      ret->updates_.emplace_back(get_value_id(a->table_name_, a->column_name_, table_schema), a->update_value_->clone());
    }

    if (statement->predicate_) {
      ret->ch_ = add_filter(std::move(read_from_table), statement->predicate_.get());
    } else {
    ...
    ret->table_name_ = statement->table_name_;
    table_id_table_.push_back(total_table_num_++);
    ret->output_schema_.Append(OutputColumnData{column_id_++, "", "updated rows", FieldType::INT64, 0});
  ...
  std::unique_ptr<PlanNode> plan_delete(DeleteStatement* statement) {
    ...
    if (statement->predicate_) {
      ret->ch_ = add_filter(std::move(seq_scan_plan), statement->predicate_.get());
    } else {
    ...
    ret->table_bitset_ = ret->ch_->table_bitset_;
    table_id_table_.push_back(total_table_num_++);
    ret->output_schema_.Append(OutputColumnData{column_id_++, "", "deleted rows", FieldType::INT64, 0});
    return ret;
  }
  ...
  std::unique_ptr<PlanNode> plan_select(SelectStatement* statement) {
    ...
    // Generate plans for tables and generate a naive join plan over them (i.e. connect them one by one)
    ...
    for (auto& a : statement->tables_) {
      ...
      if (!read_from_tables)
        ...
        read_from_tables = join_two_plan(std::move(read_from_tables), std::move(plan));
    }

    // The output schema of read_from_tables are the concats of table_schema.GetStorageColumn()
    // But if we use * in result columns, we want the concats of table_schema.GetColumn()
    // This is the concats of table_schema.GetStorageColumn().
    OutputSchema concat_get_column;
    if (read_from_tables) {
      concat_get_column.GetCols().reserve(read_from_tables->output_schema_.Size());
      get_table_schema_concat(read_from_tables.get(), concat_get_column);
    }

    // For every result column, if it is * (type_ == ResultColumnType::ALL), then we
    // enumerate all the columns of tables and add ColumnExpr to output_exprs_
    // Otherwise, its type is ResultColumnType::EXPR
    // We first call analysis_expr to analysis the return type and variables.
    // Then add it to column_name_table_ using column name _#1, _#2...
    ...
    // Check whether it has aggregate functions (then we should use AggregatePlanNode)
    bool aggregate_flag = false;

    bool has_non_trivial_output = false;
    auto current_table_num = total_table_num_;

    // Do not check whether the column names are the same. Unless they are used in expressions.
    // For example, we allow:
    //      select * from A, A;
    //      select * from (select 1 as b) as c, (select 2 as b) as c;
    // We do not allow:
    //      select a + b from (select 1 as b) as c, (select 1 as b) as c, (select 2 as a) as c;
    // (here b is ambiguous.)
    for (auto& a : statement->result_column_) {
      if (a->type_ == ResultColumnType::ALL) {
        if (a->as_ != "") DB_ERR("Internal Error: \'*\' cannot have alias.");
        if (!read_from_tables) throw PlannerException("Variables exist but no input tables.");
        output_schema.Append(concat_get_column);
        for (auto& output_column : concat_get_column.GetCols()) {
          auto expr = std::make_unique<ColumnExpr>(output_column.table_name_, output_column.column_name_);
        ...
        // Use column names from referred tables and do not use aliases from previous result column.
        // An example:
        //      select 1 as a, a + 1;
        // This is wrong.
        ...
        if (expr->expr_->type_ == ExprType::COLUMN) {
          ...
          auto id =
              get_value_id(this_expr->table_name_, this_expr->column_name_, read_from_tables ? read_from_tables->output_schema_ : OutputSchema());
          if (a->as_ == "") {
            // read_from_tables must be not nullptr. Otherwise it must throw an exception before.
          ...
          this_expr->ret_type_ = trans_to_ret_type(read_from_tables->output_schema_[id].type_);
          this_expr->id_in_column_name_table_ = read_from_tables->output_schema_[id].id_;
          this_expr->id_table_in_planner_ = table_id_table_[read_from_tables->output_schema_[id].id_];
        } else {
          auto expr_result = analysis_expr(expr->expr_.get(), read_from_tables ? read_from_tables->output_schema_ : OutputSchema());
          aggregate_flag |= expr_result.aggregate_expr_num > 0;
          auto column_name = a->as_ == "" ? fmt::format("_#{}", ++unname_col_) : a->as_;
          // size_ = 0 means that the field is a StaticFieldRef.
          // It is calculated and stored in an allocated memory.
          // size_ > 0 means that the field is in a tuple which is stored in the raw page and has not been deserialized.
          if (!has_non_trivial_output) {
            has_non_trivial_output = true;
            total_table_num_++;
          }
          table_id_table_.push_back(current_table_num);
          output_schema.Append(OutputColumnData{column_id_++, "", column_name, trans_to_field_type(expr->expr_->ret_type_), 0});
        }
    ...
    if (read_from_tables == nullptr) {
      ...
      table->values_ = std::make_shared<StaticFieldVector>(std::vector<Field>{Field::CreateInt(FieldType::INT64, 8, 0)});

      table_id_table_.push_back(current_table_num);
      table->output_schema_.Append(OutputColumnData{column_id_++, "", "unused", FieldType::INT64, 0});
    ...
    // If there is a predicate (where clause), create a filter on read_from_tables.
    if (statement->predicate_) {
      read_from_tables = add_filter(std::move(read_from_tables), statement->predicate_.get());
    }
    ...
    // Group by clause can be used even when there is no aggregate functions in result columns.
    // p.s. In MySQL grammar, group by 1 means group by the first result column,
    // But here it means the integer 1.
    if (aggregate_flag) {
      ...
      for (auto& a : statement->group_by_) {
        ...
        if (expr_result.aggregate_expr_num > 0) {
          throw PlannerException("Aggregate functions cannot be in group by clause.");
        }
      ...
      if (statement->having_) {
        ...
        aggr_plan->group_predicate_ = PredicateVec::Create(statement->having_.get());
      }
    ...
    // We evaluate the order by expression in ProjectPlanNode or AggregatePlanNode.
    if (statement->order_by_.size()) {
      ...
      for (auto& a : statement->order_by_) {
        ...
        if (expr_result.aggregate_expr_num > 0 && !aggregate_flag) {
          throw PlannerException("Aggregate functions cannot be in order by clause unless there exists group by clause.");
        }
        // Create new expressions for OrderBy.
        order_by_value_exprs.push_back(a->expr_->clone());
        auto column_name = fmt::format("_#{}", ++unname_col_);
        table_id_table_.push_back(current_table_num);
        new_output_schema.Append(OutputColumnData{column_id_++, "", column_name, trans_to_field_type(a->expr_->ret_type_), 0});
      ...
      ret->output_schema_.GetCols().insert(ret->output_schema_.GetCols().begin(), std::make_move_iterator(new_output_schema.GetCols().begin()),
                                           std::make_move_iterator(new_output_schema.GetCols().end()));
      if (aggregate_flag) {
        static_cast<AggregatePlanNode*>(ret.get())->output_exprs_.insert(static_cast<AggregatePlanNode*>(ret.get())->output_exprs_.begin(),
                                                                         std::make_move_iterator(order_by_value_exprs.begin()),
                                                                         std::make_move_iterator(order_by_value_exprs.end()));

      } else {
        static_cast<ProjectPlanNode*>(ret.get())->output_exprs_.insert(static_cast<ProjectPlanNode*>(ret.get())->output_exprs_.begin(),
                                                                       std::make_move_iterator(order_by_value_exprs.begin()),
                                                                       std::make_move_iterator(order_by_value_exprs.end()));
      }
    ...
    if (statement->limit_count_) {
      auto limit_count_result = analysis_expr(statement->limit_count_.get(), input_schema);
      if (!limit_count_result.is_constant_) {
        throw PlannerException("We only support constant limit count.");
      }
      // Has been constant folded.
      if (statement->limit_count_->ret_type_ != RetType::INT || statement->limit_count_->type_ != ExprType::LITERAL_INTEGER) {
        throw PlannerException("Limit count must be integer.");
      }
      auto limit_plan = std::make_unique<LimitPlanNode>();
      limit_plan->limit_size_ = static_cast<const LiteralIntegerExpr*>(statement->limit_count_.get())->literal_value_;
      if (statement->limit_offset_) {
        auto limit_offset_result = analysis_expr(statement->limit_offset_.get(), input_schema);
        if (!limit_offset_result.is_constant_) {
          throw PlannerException("We only support constant limit offset.");
        }
        // Has been constant folded.
        if (statement->limit_offset_->ret_type_ != RetType::INT || statement->limit_count_->type_ != ExprType::LITERAL_INTEGER) {
          throw PlannerException("Limit offset must be integer.");
        }
        limit_plan->offset_ = static_cast<const LiteralIntegerExpr*>(statement->limit_offset_.get())->literal_value_;
      }
  ...
  // Plan seq scan of a table. It records the table name. The table name will be used to get corresponding iterators from DB.
  // It adds all the columns to the column name table.
  std::unique_ptr<SeqScanPlanNode> plan_seq_scan(NormalTableRef* table) {
    ...
    if (!table_id.has_value()) throw PlannerException(fmt::format("Table \'{}\' does not exist.", table->table_name_));
    ...
    for (uint32_t index = 0; const auto& a : table_data.GetStorageColumns()) {
      ...
      ret->output_schema_[index] = OutputColumnData{column_id_++, ret->table_name_, a.name_, a.type_, a.size_};
      index += 1;
    }
  ...
  std::unique_ptr<PlanNode> add_filter(std::unique_ptr<PlanNode> child, Expr* expr) {
  ...
  // Plan join of two tables. The output schema is the union of the schemas of two tables.
  // If the on clause is not specified, the predicate is set to be nullptr.
  ...
  std::unique_ptr<PlanNode> plan_values_table(ValuesTableRef* table) {
    ...
    for (uint32_t index = 0; auto& a : table->values_) {
      if (index < table->num_fields_per_tuple_) {
        table_id_table_.push_back(current_table_num);
        ret->output_schema_.Append(OutputColumnData{column_id_++, "", fmt::format("_#{}", ++unname_col_), a.type_, 0});
      } else {
        if (!IsTypeEqual(a.type_, ret->output_schema_[index % table->num_fields_per_tuple_].type_)) {
          throw PlannerException(fmt::format("The type of the {}-th field in the {}-th tuple in values clause is not correct.",
                                             index % table->num_fields_per_tuple_ + 1, index / table->num_fields_per_tuple_ + 1));
        }
      }
      index += 1;
    }
  ...
  std::unique_ptr<PlanNode> plan_table(TableRef* table) {
    ...
    if (table->as_ != nullptr) {
      if (table->as_->column_names_.size() == 0) {
        for (auto& col : ret->output_schema_.GetCols()) col.table_name_ = table->as_->table_name_;
      } else {
        if (ret->output_schema_.Size() != table->as_->column_names_.size()) {
          throw PlannerException("Number of columns in the AS clause is not correct.");
        }
  ...
  void get_table_schema_concat(PlanNode* table, OutputSchema& vec) {
      ...
      if (table_data.GetHidePKFlag()) {
        ...
        for (uint32_t i = 0; i < this_table->output_schema_.Size(); i++)
          if (i != table_data.GetPrimaryKeyIndex()) {
            this_vec.GetCols().push_back(this_table->output_schema_[table_data.GetShuffleToStorage()[i]]);
          }
      } else {
        this_vec.GetCols().resize(this_table->output_schema_.Size());
        for (uint32_t i = 0; i < this_table->output_schema_.Size(); i++) {
          this_vec[i] = this_table->output_schema_[table_data.GetShuffleToStorage()[i]];
        }
      }
    ...
    } else if (table->type_ == PlanType::Project || table->type_ == PlanType::Aggregate || table->type_ == PlanType::Print ||
               table->type_ == PlanType::Order) {
      vec.Append(table->output_schema_);
    } else if (table->type_ == PlanType::Filter) {
      return get_table_schema_concat(static_cast<FilterPlanNode*>(table)->ch_.get(), vec);
    } else if (table->type_ == PlanType::Limit) {
      return get_table_schema_concat(static_cast<LimitPlanNode*>(table)->ch_.get(), vec);
    } else if (table->type_ == PlanType::Distinct) {
      return get_table_schema_concat(static_cast<DistinctPlanNode*>(table)->ch_.get(), vec);
    } else {
   ...
   *    A     B
   */
  std::unique_ptr<PlanNode> join_two_plan(std::unique_ptr<PlanNode> ch0, std::unique_ptr<PlanNode> ch1) {
    auto ret = std::make_unique<JoinPlanNode>();
...
BasicPlanGenerator::BasicPlanGenerator(const DBSchema& schema) { ptr_ = std::make_unique<Impl>(schema); }

BasicPlanGenerator::~BasicPlanGenerator() {}

std::pair<std::unique_ptr<PlanNode>, std::string> BasicPlanGenerator::Plan(Statement* statement) { return ptr_->Plan(statement); }
  ...
  return fmt::format("Project [Output: {}] \n  -> {}",
                     VecToString(output_exprs_,
                                 [&](const std::unique_ptr<Expr>& x) {
                                   i++;
                                   return fmt::format("{}%{}={}", output_schema_[i - 1].column_name_, output_schema_[i - 1].id_, x->ToString());
                                 }),
...
std::string SeqScanPlanNode::ToString() const { return fmt::format("Seq Scan [Table: {}] [Predicate: {}]", table_name_, predicate_.ToString()); }
...
std::string HashJoinPlanNode::ToString() const {
  return fmt::format(
      "Join [Predicate: {}] \n  [Hash Keys: {}]\n  -> {}\n  [Hash Keys: {}]\n  -> {}", predicate_.ToString(),
      VecToString(left_hash_exprs_, [&](const std::unique_ptr<Expr>& x) { return x->ToString(); }), AddSpacesAfterNewLine(ch_->ToString(), 4),
      VecToString(right_hash_exprs_, [&](const std::unique_ptr<Expr>& x) { return x->ToString(); }), AddSpacesAfterNewLine(ch2_->ToString(), 4));
}

std::string AggregatePlanNode::ToString() const {
  int i = 0;
  return fmt::format("Aggregate [Group by: {}] [Group predicate: {}] [Output: {}] \n  -> {}",
                     VecToString(group_by_exprs_, [](const std::unique_ptr<Expr>& x) { return x->ToString(); }), group_predicate_.ToString(),
                     VecToString(output_exprs_,
                                 [&](const std::unique_ptr<Expr>& x) {
                                   i++;
                                   return fmt::format("{}%{}={}", output_schema_[i - 1].column_name_, output_schema_[i - 1].id_, x->ToString());
  ...
  return fmt::format("Sort [On: {}] \n  -> {}",
                     VecToString(order_by_exprs_,
                                 [&](const std::pair<RetType, bool>& x) {
                                   auto expr = std::make_unique<ColumnExpr>(ch_->output_schema_[i].table_name_, ch_->output_schema_[i].column_name_);
                                   expr->id_in_column_name_table_ = ch_->output_schema_[i].id_;
                                   i++;
                                   return fmt::format("{} {}", expr->ToString(), x.second ? "asc" : "desc");
                                 }),
                     AddSpacesAfterNewLine(ch_->ToString(), 4));
}

std::string LimitPlanNode::ToString() const {
  return fmt::format("Limit [Limit {}, Offset {}] \n  -> {}", limit_size_, offset_, AddSpacesAfterNewLine(ch_->ToString(), 4));
}

std::string InsertPlanNode::ToString() const { return fmt::format("Insert \n  -> {}", AddSpacesAfterNewLine(ch_->ToString(), 4)); }

std::string UpdatePlanNode::ToString() const {
  return fmt::format(
      "Update [{}] \n  -> {}",
      VecToString(updates_,
                  [](const std::pair<uint32_t, std::unique_ptr<Expr>>& x) { return fmt::format("Col%{}={}", x.first, x.second->ToString()); }),
      AddSpacesAfterNewLine(ch_->ToString(), 4));
}

std::string DeletePlanNode::ToString() const { return fmt::format("Delete \n  -> {}", AddSpacesAfterNewLine(ch_->ToString(), 4)); }

std::string PrintPlanNode::ToString() const {
  return fmt::format("Print Values [{} tuples, {} fields per tuple]", values_->GetFieldVector().size() / num_fields_per_tuple_,
...
std::unique_ptr<PlanNode> ProjectPlanNode::clone() const {
  ...
  for (auto& a : output_exprs_) ret->output_exprs_.push_back(a->clone());
...
std::unique_ptr<PlanNode> AggregatePlanNode::clone() const {
  ...
  ret->ch_ = ch_ ? ch_->clone() : nullptr;
  for (auto& a : output_exprs_) ret->output_exprs_.push_back(a->clone());
  for (auto& a : group_by_exprs_) ret->group_by_exprs_.push_back(a->clone());
  ret->table_bitset_ = table_bitset_;
  return ret;
}
...
std::unique_ptr<PlanNode> UpdatePlanNode::clone() const {
  ...
  ret->output_schema_ = output_schema_;
  ret->table_name_ = table_name_;
  for (auto& [col, expr] : updates_) ret->updates_.push_back({col, expr->clone()});
  ret->ch2_ = ch2_ ? ch2_->clone() : nullptr;
  ret->ch_ = ch_ ? ch_->clone() : nullptr;
  ret->table_bitset_ = table_bitset_;
  return ret;
...
std::string ColumnSchema::ToString() const { return fmt::format("{} {}({})", name_, field_type_str[static_cast<uint32_t>(type_)], size_); }

std::string ForeignKeySchema::ToString() const { return fmt::format("{} -> {}({})", name_, table_name_, column_name_); }

std::string TableSchema::ToString() const {
  return fmt::format("{} [{}] primary key [{}] foreign key [{}]", name_, VecToString(columns_, [&](const ColumnSchema& x) { return x.ToString(); }),
                     columns_[pk_index_].name_, VecToString(fk_, [&](const ForeignKeySchema& x) { return x.ToString(); }));
}
