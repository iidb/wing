...
/**
 * 
 * Joins are initially NestloopJoins. But for some joins we can transform it to hash join.
 * These joins have at least one predicate that the two expressions only use data from one table.
 * For example. 
 * select * from A, B where A.a = B.a;
 * 
 * For the following SQL statement, 'A.a * 2 + A.b' and 'B.c' will be used as hash keys. 
 * Although 'A.a + B.b = 1' can be transformed to 'A.a = 1 - B.b', we don't consider that (This may be done by other rules.)
 * select * from A, B where A.a * 2 + A.b = B.c and A.a + B.b = 1;
 * 
 * If there are multiple equalities, all of them will be used as hash keys.
 * For the following SQL statement, hash keys for A are 'A.a' and 'A.b', for B are 'B.a' and 'B.b'.
    ...
    if (node->type_ == PlanType::Join) {
      ...
      for (auto& a : t_node->predicate_.GetVec()) {
        if (a.expr_->op_ == OpType::EQ) {
          if (!a.CheckRight(t_node->ch_->table_bitset_) && !a.CheckLeft(t_node->ch2_->table_bitset_) && a.CheckRight(t_node->ch2_->table_bitset_) &&
              a.CheckLeft(t_node->ch_->table_bitset_)) {
            return true;
          }
          if (!a.CheckLeft(t_node->ch_->table_bitset_) && !a.CheckRight(t_node->ch2_->table_bitset_) && a.CheckRight(t_node->ch_->table_bitset_) &&
  ...
  std::unique_ptr<PlanNode> Transform(std::unique_ptr<PlanNode> node) {
    ...
    for (auto&& a : t_node->predicate_.GetVec()) {
      if (a.expr_->op_ == OpType::EQ) {
        ...
        } else if (!a.CheckLeft(t_node->ch_->table_bitset_) && !a.CheckRight(t_node->ch2_->table_bitset_) &&
                   a.CheckRight(t_node->ch_->table_bitset_) && a.CheckLeft(t_node->ch2_->table_bitset_)) {
